...
var excluded = map[string]string{
	...
	"update_check": "the parity installation has no release feed configured (BaseURL " +
		"empty), so this could only exercise the not-configured answer, never a real " +
	...
	"update_apply": "restarts the daemon out from under whichever surface is running it — " +
		...
		"Covered by the update suite's own Apply tests over a fake DaemonSupervisor, and " +
		"against the real gateway inside the daemon by " +
		"TestApplyInsideTheDaemonRefusesBeforeTouchingAnything.",
}
