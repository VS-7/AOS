 ...
 * builds `FractalAppError` via `FractalError.create().merge(...).build()` —
 ...
 *
 * Every consumer of this file only does `error instanceof FractalAppError`
 ...
 * nothing currently constructs a `FractalAppError` instance from the
 ...
 */
export class FractalAppError extends Error {
  code?: string;
  cta?: unknown;

  constructor(message: string, code?: string) {
    super(message);
    this.name = "FractalAppError";
    this.code = code;
  }
}
