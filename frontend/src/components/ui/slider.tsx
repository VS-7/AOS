...
const Slider = forwardRef<HTMLDivElement, SliderProps>(
  (
    ...
    useLayoutEffect(() => {
      ...
      const px0 = valueToPixel(v0, min, max, w);
      motionX0.set(px0);
    ...
    useEffect(() => {
      ...
      const ro = new ResizeObserver(([entry]) => {
        ...
        if (!dragging.current && initialSyncDone.current) {
          ...
          const v0 = v[0];
          if (v0 === undefined) return;
          const px0 = valueToPixel(v0, mn, mx, w);
    ...
    useEffect(() => {
      ...
      const v0 = values[0];
      if (v0 === undefined) return;
      const px0 = valueToPixel(v0, min, max, tw);
      animate(motionX0, px0, springs.moderate);
      if (isRange && values[1] !== undefined) {
    ...
    const renderVisualThumb = (index: number) => {
      ...
      return (
        ...
        >
          ...
          {/* Focus ring */}
          <motion.span
            className="absolute rounded-md border border-[#6B97FF] pointer-events-none"
            initial={false}
            animate={{
      ...
      >
        ...
        >
          ...
          {showValue && valuePosition === "tooltip" && (
            <AnimatePresence>
              {isInteracting && values[0] !== undefined && (
                <TooltipValue
                  key="tooltip-0"
                  value={values[0]}
                  formatValue={formatValue}
          ...
          >
            ...
            {/* Track background */}
            <motion.div
              className={cn("absolute border border-border overflow-hidden rounded-md", trackClassName)}
              initial={false}
              animate={{
  ...
  (
    ...
    return (
      ...
      >
        ...
        {variant === "pips" && (
          <motion.div
            className="absolute rounded-md pointer-events-none z-[3]"
            initial={false}
            animate={{
              top: isActive ? 7 : 8,
              bottom: isActive ? 7 : 8,
