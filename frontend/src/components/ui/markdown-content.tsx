...
function parseTaggedLinkContent(raw: string): TaggedLinkData | null {
  ...
  if (markdownLinkMatch) {
    // Both capture groups are mandatory in the regex, so they are always
    // present when the overall match succeeds.
    return {
...
interface TaskCardPayloadV1 {
  ...
  type?: string;
  chat?: string | null;
  worktree?: { enabled?: boolean; branch?: string | null; path?: string | null };
  todos?: Array<{ id?: string; description?: string; status?: string; agent?: string }>;
  todosMeta?: { total?: number; hidden?: number };
}

type CustomInlineTagBlock =
  | { id: string; kind: 'mention'; raw: string; idValue?: string }
  | { id: string; kind: 'source'; raw: string; name?: string; path?: string; sourceType?: 'file' | 'folder' | 'skill' }
  | { id: string; kind: 'rich-text'; raw: string; text?: string }
  | { id: string; kind: 'task-card'; raw: string; payload?: TaskCardPayloadV1; parseError?: string };
      ...
      {isCollapsed ? (
        <div className="relative cursor-pointer" onClick={toggleCollapse}>
          ...
          <div className="absolute inset-x-0 bottom-0 flex items-center justify-center pb-2 pointer-events-none">
            <span className="font-pixel text-[9px] text-muted-foreground/50 bg-accent/80 px-2.5 py-1 rounded-md flex items-center gap-1 tracking-wider">
              <ChevronDown size={10} />
...
const useProcessedContent = (content: string) => {
  return useMemo(() => {
    ...
    const customInlineTags: CustomInlineTagBlock[] = [];
    ...
    try {
      ...
      // Extract AOS custom inline tags before markdown/citation transforms.
      const customTagPattern =
        /<(chat-mention|chat-source|chat-inline-rich-text|task-card)\b([^>]*)>([\s\S]*?)<\/\1>/gi;
      let customProcessed = '';
      let lastCustomIndex = 0;
      let customMatch: RegExpExecArray | null;

      while ((customMatch = customTagPattern.exec(modifiedContent)) !== null) {
        const [raw, tagName, rawAttrs, innerContent] = customMatch;
        const attrs = parseTagAttributes(rawAttrs ?? '');
        const id = `XCUSTOMTAGX${customInlineTags.length}XEND`;

        if (tagName === 'chat-mention') {
          customInlineTags.push({
            id,
            kind: 'mention',
            raw,
            idValue: attrs.id?.trim() || undefined,
          });
        } else if (tagName === 'chat-source') {
          const typeValue =
            attrs.type === 'file' || attrs.type === 'folder' || attrs.type === 'skill'
              ? attrs.type
              : undefined;
          customInlineTags.push({
            id,
        ...
        } else if (tagName === 'chat-inline-rich-text') {
          customInlineTags.push({
            id,
            kind: 'rich-text',
            raw,
          ...
          let payload: TaskCardPayloadV1 | undefined;
          let parseError: string | undefined;
          try {
            if (!attrs.data) {
              throw new Error('missing-data');
            }
            payload = JSON.parse(attrs.data) as TaskCardPayloadV1;
            if (payload.version !== 'v1') {
              throw new Error('invalid-version');
            }
          } catch (error) {
            parseError = error instanceof Error ? error.message : 'invalid-payload';
          }
        ...
        customProcessed += modifiedContent.slice(lastCustomIndex, customMatch.index) + id;
        lastCustomIndex = customMatch.index + raw.length;
      }

      customProcessed += modifiedContent.slice(lastCustomIndex);
      modifiedContent = customProcessed;

      // If this is an XML context payload carrying visual custom tags, keep only
      // the visual placeholders so users see cards/tags instead of raw XML.
      if (modifiedContent.includes('<context>') && modifiedContent.includes('XCUSTOMTAGX')) {
        const visualPlaceholders = modifiedContent.match(/XCUSTOMTAGX\d+XEND/g) ?? [];
        if (visualPlaceholders.length > 0) {
          modifiedContent = visualPlaceholders.join('\n\n');
        }
      }
      ...
      if (typeof tableBlocks !== 'undefined' && tableBlocks.length > 0) {
        for (let t = 0; t < tableBlocks.length; t++) {
          const tableBlock = tableBlocks[t];
          if (!tableBlock) continue;
          let rowContent = tableBlock.content;
          ...
          if (/^\s*$/.test(between)) {
            ...
            if (currentGroup.length >= 2) {
              ...
              if (firstInGroup && lastInGroup) {
                groups.push({
                  links: currentGroup,
                  startIndex: firstInGroup.index,
                  endIndex: lastInGroup.end,
                });
              }
            }
            // Start new group
      ...
      if (typeof tableBlocks !== 'undefined' && tableBlocks.length > 0) {
        for (let t = 0; t < tableBlocks.length; t++) {
          ...
          // Guaranteed defined: rowLinks.length >= 2 was just checked above.
      ...
      monetaryBlocks.forEach(({ id, content }) => {
        ...
        for (const citation of citations) {
          citation.text = citation.text.replace(regex, content);
        }
        ...
        for (const latexBlock of latexBlocks) {
          latexBlock.content = latexBlock.content.replace(regex, content);
        }
      });
      ...
      tableBlocks.forEach(({ id, content }) => {
        ...
        for (const citation of citations) {
          citation.text = citation.text.replace(tregex, restoredRow);
        }
      });
...
const TaskCardInline: React.FC<{ payload?: TaskCardPayloadV1; parseError?: string }> = React.memo(
  ({ payload, parseError }) => {
    if (!payload || !payload.name) {
      return (
        <span className="my-2 inline-flex rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
    ...
    const href = payload.id ? `/tasks/${encodeURIComponent(payload.id)}` : null;

    const card = (
      <span className="my-2 inline-flex min-w-72 max-w-full flex-col gap-2 rounded-md border bg-secondary p-4 align-middle text-left">
        <span className="flex items-center justify-between gap-2">
          <span className="truncate text-sm font-semibold text-foreground">
            {payload.id ? `${payload.id} - ${payload.name}` : payload.name}
          </span>
        </span>

        {payload.summary ? (
          <span className="line-clamp-2 text-xs text-muted-foreground">{payload.summary}</span>
        ) : null}
      </span>
    );
    ...
    return (
      <a className="inline-block max-w-full no-underline hover:no-underline" href={href}>
        {card}
      </a>
    );
  },
...
TaskCardInline.displayName = 'TaskCardInline';
  ...
  return (
    <span className="inline-flex w-full max-w-full align-middle my-1">
      <a
        ...
        className="group inline-flex w-full max-w-105 flex-col overflow-hidden rounded-xl border border-border/60 bg-muted/40 no-underline transition-colors hover:bg-muted"
      >
        <span
        ...
        <span className="flex items-center gap-2 px-3 py-2.5">
          <span className="flex size-8 shrink-0 items-center justify-center rounded-md border border-border/50 bg-background text-foreground/80">
...
const MobileHoverCard: React.FC<{
  ...
  const linkClassName = isCitation
    ? 'cursor-pointer inline-flex items-center align-middle text-[10px] leading-tight font-medium no-underline px-1.5 py-0.5 mb-0.5! m-0! rounded-md bg-muted/60 text-muted-foreground hover:bg-muted hover:text-foreground active:bg-muted transition-colors focus-visible:ring-2 focus-visible:ring-ring/40 outline-none touch-manipulation'
    ...
    return (
      <HoverCard open={isOpen} onOpenChange={handleOpenChange}>
        <HoverCardTrigger asChild>
          <a href={href} target="_blank" rel="noopener noreferrer" onClick={handleClick} className={linkClassName}>
            {text}
          </a>
        </HoverCardTrigger>
        <HoverCardContent
          ...
          align="start"
          sideOffset={6}
          className="w-60 p-0 shadow-md border border-border/60 rounded-md overflow-hidden bg-popover"
...
const CitationItem = React.memo(
  ({
    ...
    const renderIcon = () => {
      if (platform === 'youtube') {
        return (
          <div className="w-3.5 h-3.5 flex items-center justify-center text-muted-foreground">
            <YoutubeLogoIcon size={14} />
          </div>
        );
      }
      if (platform === 'spotify') {
...
const CitationGroup = React.memo(({ urls, texts, elementKey }: CitationGroupProps) => {
  ...
  const firstDomain = useMemo(() => {
    ...
    try {
      ...
      return getDisplayDomain(firstUrl);
    }
  }, [urls]);
  ...
  const triggerContent = (
    <span
      ...
      className="cursor-pointer inline-flex items-center align-middle gap-1 text-[10px] leading-tight font-medium no-underline px-1.5 py-0.5 mb-0.5! m-0! rounded-md bg-muted/60 text-muted-foreground hover:bg-muted hover:text-foreground active:bg-muted transition-colors focus-visible:ring-2 focus-visible:ring-ring/40 outline-none touch-manipulation"
    >
    ...
    return (
      <HoverCard open={isOpen} onOpenChange={handleOpenChange}>
        ...
        <HoverCardContent
          ...
          align="start"
          sideOffset={8}
          className="w-64 p-0 shadow-md border border-border/60 rounded-md overflow-hidden bg-popover"
          onClick={(e) => e.stopPropagation()}
        >
          {dropdownContent}
        </HoverCardContent>
...
const FileLinkPreview = React.memo(({ href, title }: { href: string; title?: string }) => {
  ...
  return (
    <span className="inline-flex max-w-full align-middle my-1">
      <span className="inline-flex max-w-90 items-center gap-2 rounded-xl border border-border/60 bg-muted/40 px-2.5 py-2">
        <a
          href={href}
          target="_blank"
          rel="noopener noreferrer"
  ...
  return (
    <span className="block my-2 relative group" style={{ overflowAnchor: 'none' }}>
      <Cambio.Root motion="smooth">
        <Cambio.Trigger
          className="w-full rounded-md border border-border overflow-hidden cursor-zoom-in block bg-muted/30"
          ...
          <Cambio.Popup className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="relative">
              <img
                ...
                draggable={false}
              />
              <Cambio.Close className="absolute top-2 right-2 p-1.5 rounded-md bg-black/50 text-white backdrop-blur-sm hover:bg-black/70 transition-colors cursor-pointer">
...
const MarkdownRenderer: React.FC<MarkdownRendererProps> = React.memo(
  ({ content, isUserMessage = false }) => {
    ...
    const renderCitation = useCallback(
      (_index: number, citationText: string, href: string, key: string) => {
        ...
        if (platform === 'youtube' || platform === 'spotify') {
          const platformConfig =
            platform === 'youtube'
              ? { name: 'YouTube', icon: <YoutubeLogoIcon className="w-3 h-3" /> }
            ...
            <HoverCard key={key} openDelay={10}>
              <HoverCardTrigger asChild>
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center align-middle gap-1.5 px-1.5 py-0.5 mb-0.5! rounded-md text-[10px] leading-tight font-medium no-underline transition-colors bg-muted/60 text-muted-foreground hover:bg-muted hover:text-foreground"
                >
                  {platformConfig.icon}
    ...
    const renderStructuredSegments = useCallback((textValue: string) => {
      // Check if text contains any LaTeX patterns or citation groups without mutating regex state
      const hasLatex = textValue.includes('XLATEXBLOCKX') || textValue.includes('XLATEXINLINEX');
      const hasCitationGroup = textValue.includes('XCITATIONGRPX');
      const hasAppPreview = textValue.includes('XAPPPREVX');
      const hasDownload = textValue.includes('XDOWNLOADX');
      const hasCustomTag = textValue.includes('XCUSTOMTAGX');

      if (!hasLatex && !hasCitationGroup && !hasAppPreview && !hasDownload && !hasCustomTag) {
        return textValue;
      }
      ...
      const customTagPattern = /XCUSTOMTAGX(\d+)XEND/g;

      const components: any[] = [];
      let lastEnd = 0;
      const allMatches: Array<{
        match: RegExpExecArray;
        type: 'latex-block' | 'latex-inline' | 'citation-group' | 'app-preview' | 'download' | 'custom-tag';
      }> = [];

      let match;
      while ((match = blockPattern.exec(textValue)) !== null) {
        allMatches.push({ match, type: 'latex-block' });
      }
      while ((match = inlinePattern.exec(textValue)) !== null) {
        allMatches.push({ match, type: 'latex-inline' });
      }
      while ((match = citationGroupPattern.exec(textValue)) !== null) {
        allMatches.push({ match, type: 'citation-group' });
      }
      while ((match = appPreviewPattern.exec(textValue)) !== null) {
        allMatches.push({ match, type: 'app-preview' });
      }
      while ((match = downloadPattern.exec(textValue)) !== null) {
        allMatches.push({ match, type: 'download' });
      }
      while ((match = customTagPattern.exec(textValue)) !== null) {
        allMatches.push({ match, type: 'custom-tag' });
      }
      ...
      allMatches.forEach(({ match, type }) => {
        ...
        if (start > lastEnd) {
          const textContent = textValue.slice(lastEnd, start);
        ...
        if (type === 'citation-group') {
          ...
          if (citationGroup) {
            ...
            components.push(<CitationGroup key={key} urls={citationGroup.urls} texts={citationGroup.texts} elementKey={key} />);
          }
        ...
        } else if (type === 'custom-tag') {
          const customTag = customInlineTags.find((tag) => tag.id === fullMatch);
          if (customTag) {
            const key = getElementKey('text', customTag.id);

            if (customTag.kind === 'mention') {
              if (customTag.idValue) {
                components.push(<ChatInlineMentionTag key={key} className="mx-0.5 align-middle" id={customTag.idValue} />);
              } else {
                components.push(<span key={key}>{customTag.raw}</span>);
              }
            } else if (customTag.kind === 'source') {
              if (customTag.name && customTag.sourceType) {
                components.push(
                  <ChatInlineSourceTag
                    key={key}
                    ...
                    name={customTag.name}
                    path={customTag.path ?? ''}
                    sourceType={customTag.sourceType}
                  />,
                );
              } else {
                components.push(<span key={key}>{customTag.raw}</span>);
              }
            } else if (customTag.kind === 'rich-text') {
              components.push(
                <ChatInlineRichText
                  key={key}
                  className="mx-0.5 align-middle"
                  inlineTagClassName="mx-0.5 align-middle"
                  text={customTag.text ?? ''}
                />,
              );
            } else {
              components.push(<TaskCardInline key={key} payload={customTag.payload} parseError={customTag.parseError} />);
            }
          }
        } else {
          ...
          if (latexBlock) {
            ...
            if (type === 'latex-block') {
              components.push(
                <SafeLatex key={key} delimiters={[{ left: '$$', right: '$$', display: true }, { left: '\\[', right: '\\]', display: true }]} isBlock={true}>
                  {latexBlock.content}
                </SafeLatex>,
              );
            } else {
              components.push(
                <SafeLatex key={key} delimiters={[{ left: '$', right: '$', display: false }, { left: '\\(', right: '\\)', display: false }]} isBlock={false}>
                  {latexBlock.content}
                </SafeLatex>,
              );
            }
      ...
      if (lastEnd < textValue.length) {
        const textContent = textValue.slice(lastEnd);
        const key = getElementKey('text', textContent);
        components.push(<span key={key}>{textContent}</span>);
      }

      return components.length === 1 ? components[0] : <Fragment>{components}</Fragment>;
    }, [appPreviewBlocks, citationGroups, customInlineTags, downloadBlocks, getElementKey, latexBlocks]);

    const renderer: Partial<ReactRenderer> = useMemo(
      () => ({
        text(text: string) {
          return renderStructuredSegments(text);
        },
        html(html: string) {
          return renderStructuredSegments(html);
        },
        hr() {
          return <></>;
        },
        ...
        link(href, text) {
          ...
          // citationIndex is guaranteed valid here: it's either the result of a
          // successful findIndex or the index of the entry just pushed above.
        ...
        heading(children, level) {
          ...
          return (
            <HeadingTag key={key} className={`${sizeClasses} font-bold text-foreground tracking-tight scroll-mt-20`}>
              {children}
            </HeadingTag>
          );
        },
        ...
        tableCell(children, flags) {
          ...
          return isHeader ? (
            <TableHead
              ...
              className={cn(
                alignClass,
                'text-[15px] border-r border-foreground/10 last:border-r-0 bg-foreground/6 font-semibold p-2! m-1! whitespace-normal wrap-break-word min-w-30',
              )}
            >
              {childrenWithKeys}
            </TableHead>
            ...
            <TableCell
              ...
              className={cn(
                alignClass,
                'text-[15px] border-r border-foreground/10 last:border-r-0 p-2! m-1! whitespace-normal wrap-break-word min-w-30',
              )}
            >
              {childrenWithKeys}
            </TableCell>
        ...
        <div className="markdown-body prose prose-neutral dark:prose-invert max-w-none text-foreground font-sans">
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <div className="animate-spin h-4 w-4 border-2 border-primary border-t-transparent rounded-md"></div>
...
export const CopyButton = React.memo(({ text }: { text: string }) => {
  ...
  return (
    <Button variant="ghost" size="sm" onClick={handleCopy} className="h-8 px-2 text-xs rounded-md">
