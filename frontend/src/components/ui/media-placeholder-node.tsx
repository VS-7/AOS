    ...
    const { openFilePicker } = useFilePicker({
      ...
      onFilesSelected: (data: SelectedFilesOrErrors<undefined, unknown>) => {
        ...
        if (restFiles.length > 0) {
          // insert.media wants a FileList, which has no public constructor —
          // DataTransfer is the standard way to build one from an array.
          const transfer = new DataTransfer();
          restFiles.forEach((file) => transfer.items.add(file));
