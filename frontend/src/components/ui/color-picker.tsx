...
function parseColor(input: string): { r: number; g: number; b: number; a: number } | null {
  ...
  if (rgbM && rgbM[1] !== undefined) {
    ...
    const p3 = parts[3];
    if (p3 !== undefined) {
      a = p3.endsWith("%") ? parseFloat(p3) / 100 : parseFloat(p3);
    }
  ...
  if (hslM && hslM[1] !== undefined) {
    ...
    const sat = p1.endsWith("%") ? parseFloat(p1) / 100 : parseFloat(p1);
    const l = p2.endsWith("%") ? parseFloat(p2) / 100 : parseFloat(p2);
    let a = 1;
    const p3 = parts[3];
    if (p3 !== undefined) {
      a = p3.endsWith("%") ? parseFloat(p3) / 100 : parseFloat(p3);
...
function SaturationSquare({ h, s, v, onChange }: SaturationSquareProps) {
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
      className="relative w-full select-none touch-none cursor-crosshair rounded-md overflow-hidden outline-none"
      style={{
...
function FormatItem({
  ...
  return (
    <div
      ...
      className={cn(
        `relative z-10 flex items-center px-3 py-2 text-[13px] cursor-pointer outline-none transition-colors duration-80 rounded-md`,
  ...
  return (
    <>
      <button
        ...
        aria-expanded={open}
        className={cn(
          "flex items-center justify-between gap-2 h-9 px-3 text-[13px] text-foreground border border-border bg-transparent hover:bg-accent/40 transition-colors duration-80 outline-none focus-visible:ring-1 focus-visible:ring-[#6B97FF] cursor-pointer rounded-md",
        )}
...
const ColorInput = forwardRef<HTMLInputElement, ColorInputProps>(
  (
    ...
    return (
      <div
        className={cn(
          "flex items-center h-9 px-3 border border-border bg-transparent transition-colors duration-80 focus-within:ring-1 focus-within:ring-[#6B97FF] rounded-md",
          className
        )}
        style={{ width }}
      >
...
function ColorTile({ color, size = 24, className, style }: ColorTileProps) {
  return (
    <span
      className={cn("inline-block relative shrink-0 overflow-hidden rounded-md", className)}
      style={{
        width: size,
        height: size,
        ...CHECKER_BG,
  ...
  ({ color, size = 28, selected, className, ...props }, ref) => {
    return (
      <button
        ...
        className={cn(
          "relative shrink-0 overflow-hidden cursor-pointer outline-none transition-shadow duration-100 rounded-md",
          className
        )}
        style={{
          width: size,
...
const ColorPicker = forwardRef<HTMLDivElement, ColorPickerProps>(
  (
    ...
    return (
      <div
        ref={ref}
        className={cn("flex flex-col gap-3 p-3 bg-card border border-border/60 rounded-md", className)}
    ...
    return (
      <div ref={ref} className="inline-flex">
        <button
          ...
          aria-expanded={open}
          className={cn(
            "flex items-center gap-2 h-9 px-2 border border-border bg-transparent hover:bg-accent/40 transition-colors duration-80 outline-none focus-visible:ring-1 focus-visible:ring-[#6B97FF] cursor-pointer rounded-md",
            triggerClassName
          )}
          style={{ fontVariationSettings: fontWeights.medium }}
        >
