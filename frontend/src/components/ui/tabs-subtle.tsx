...
> {
  ...
  activeLabel?: boolean;
  /**
   * Overrides the selected pill background.
   * Default: `bg-muted/70 dark:bg-accent/35` — too faint on sidebar surfaces;
   * pass e.g. `bg-sidebar-accent` when mounting inside the sidebar.
   */
  indicatorClassName?: string;
  /**
   * Overrides the hover pill background.
   * Default: `bg-accent/60 dark:bg-accent/30`.
   */
    ...
    return (
      ...
      >
        <div
          ...
          {...props}
        >
          {/* Selected pill — className on TabsSubtle only styles the tablist; use indicatorClassName for the active fill */}
          {selectedRect && (
            <motion.div
              data-slot="tabs-subtle-indicator"
              className={cn(
                "absolute pointer-events-none bg-muted/70 dark:bg-accent/35",
                shape.bg,
                indicatorClassName,
              )}
              initial={false}
          ...
          <AnimatePresence>
            {hoverRect && !isHoveringSelected && selectedRect && (
              <motion.div
                data-slot="tabs-subtle-hover"
                className={cn(
                  "absolute pointer-events-none bg-accent/60 dark:bg-accent/30",
                  shape.bg,
                  hoverIndicatorClassName,
                )}
                initial={{
...
interface TabsSubtleItemProps extends HTMLAttributes<HTMLButtonElement> {
  ...
  label: string;
  index: number;
  /** Optional count — stays visible even when activeLabel collapses the text. */
  count?: string | number;
}
  ...
  ({ icon: Icon, label, index, count, className, ...props }, ref) => {
    ...
    return (
      <button
        ...
        className={cn(
          "relative z-10 flex items-center gap-1.5 px-2.5 py-1.5 cursor-pointer bg-transparent border-none outline-none",
          collapseLabel && "h-8",
          shape.bg,
          className,
        )}
