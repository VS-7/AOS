 ...
 * (`FractalArtifactSchema`, the entity) and
 * `.../services/artifact/artifact.service.ts` (the exact construction site
 * for `FractalArtifactListItem` and `FractalArtifactUrls` — the type-only
 ...
 *
 * `FractalArtifactListItem` is `{ ...artifact, urls }`
 ...
 * `artifact.trigger.ts`) happens to read. `FractalArtifactUrls` comes from
 ...
 */
export type FractalArtifactVisibility = "private" | "workspace" | "by_password";

export interface FractalArtifact {
  id: string;
  name: string;
  icon?: string;
  skill?: string;
  ...
  entrypoint: string;
  serve?: string;
  visibility: FractalArtifactVisibility;
...
export type FractalArtifactListItem = FractalArtifact & {
