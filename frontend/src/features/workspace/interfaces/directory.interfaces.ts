 ...
 * `WorkspaceDirectoryUser`/`Agent` (the type-only
 ...
 *
 * `WorkspaceDirectoryAgentProcessing`'s shape comes from
 * `ChatKindHelper.index_processing_by_agent`'s return type
 * (`chat-kind.helper.ts`, already verified byte-identical between
 * `v401/web` and `v401/server`): `{ chatId, title, kind }`. `kind` stays
 * `string` rather than importing `chat`'s `ChatKind`/`ChatKind` to
 * avoid a cross-feature import — always valid, since `ChatKind` is
...
export interface WorkspaceDirectoryAgentProcessing {
  chatId: string;
  title: string;
  kind: string;
}

export interface WorkspaceDirectoryAgent {
  id: string;
  name: string;
  image?: string;
  role?: string;
