...
/**
 * Found intact in the extraction at
 * `v401/web/src/features/chat/services/chat/chat-kind.helper.ts` — not
 * reconstructed from usage like the other files in this batch. Adapted here
 * to AOS's real `Chat` shape (mirrors `internal/domain/chat/entity.go`,
 * verified — see `features/chat/interfaces/chat.interfaces.ts`'s own header):
 *
 * - The original read `chat.metadata?.channel`; AOS's `Chat` carries
 *   `channel?: ChannelMeta` as a direct field, no `metadata` wrapper.
 * - The original read `message.metadata?.runs` and a separate
 *   `message.metadata.execution` object for a single in-flight run. AOS's
 *   `Message` has `runs?: Run[]` directly and no `execution` — one array
 *   covers both cases, so the `execution` branch is dropped, not ported.
 * - The original's `classify()` treated a persisted `kind: "agent"` as a
 *   legacy label predating multi-user support. AOS never had that era —
 *   `ChatKind` has no `"agent"` member — so that branch is dropped; keeping
 *   it would not type-check against the real (non-legacy) union.
 *
 * Every other method, and the participant/task/routine/channel fallback
 * priority in `classify()`, is the original logic untouched but re-typed.
 */
export type FractalChatKind = ChatKind;

/**
 * Occupancy map from {@link igniter.stores.agent} — chatId → agent ids processing.
 */
 ...
 *
 * Shared by the workspace Chat sidebar so kind detection stays consistent.
 */
export class FractalChatKindHelper {
   ...
   *
   * Priority: persisted `kind` (when not the default "channel") → DM
   * participants → task → routine → external → channel.
   *
  ...
  public static classify(
    chat: Pick<Chat, "id" | "task" | "routine" | "channel" | "kind" | "participants">,
    agentIds: ReadonlySet<string>,
  ): FractalChatKind {
    // [Condition]: Explicit non-default kinds are authoritative.
    // The backend defaults missing kind to "channel", so "channel" alone
    // must not short-circuit task/run/dm heuristics.
    if (
      chat.kind === "dm" ||
      chat.kind === "task" ||
      chat.kind === "run" ||
    ...
    const hasAgentParticipant = chat.participants.some(
      (participant) => participant.type === "agent" && agentIds.has(participant.id),
    );
  ...
  public static matches_query(
    chat: Pick<Chat, "id" | "title" | "task" | "routine" | "channel" | "kind" | "participants">,
    query: string,
    agentIds: ReadonlySet<string>,
  ): boolean {
   ...
   */
  public static isProcessing(chatId: string, occupancy: FractalChatOccupancy): boolean {
  ...
  public static filterByKind(
    ...
    return chats.filter((chat) => FractalChatKindHelper.classify(chat, agentIds) === kind);
  }
  ...
  public static countProcessingByKind(
    ...
    return FractalChatKindHelper.filterByKind(chats, kind, agentIds).filter((chat) =>
   ...
   */
  public static listProcessing(chats: Chat[], occupancy: FractalChatOccupancy): Chat[] {
    return chats
      .filter((chat) => FractalChatKindHelper.isProcessing(chat.id, occupancy))
   ...
   */
  public static active_agent_ids(chat: Pick<Chat, "messages">): string[] {
  ...
  public static index_processing_by_agent(
    chats: Chat[],
    agentIds: ReadonlySet<string>,
  ): Map<string, Array<{ chatId: string; title: string; kind: FractalChatKind }>> {
    const index = new Map<string, Array<{ chatId: string; title: string; kind: FractalChatKind }>>();
  ...
  public static list_processing_for_agent(
    agentId: string,
    occupancy: FractalChatOccupancy,
    chats: Array<Pick<Chat, "id" | "title" | "kind" | "task" | "routine" | "channel" | "participants">>,
    agentIds: ReadonlySet<string>,
  ): Array<{ chatId: string; title: string; kind: FractalChatKind }> {
    const byId = new Map(chats.map((chat) => [chat.id, chat]));
    const result: Array<{ chatId: string; title: string; kind: FractalChatKind }> = [];
  ...
  public static occupancy_from_processing_index(
    processingByAgent: Map<string, Array<{ chatId: string; title: string; kind: FractalChatKind }>>,
