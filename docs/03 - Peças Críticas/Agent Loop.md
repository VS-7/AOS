...
**Pendente, nota corrigida:** `SubagentStart` e `SubagentStop` existem no contrato e não têm ponto de disparo. A nota original apontava para a Fase 6 como quem ia trazer o ponto de disparo; a Fase 6 já foi entregue (`v0.7.0-fase6`, ver [[Task (Go)]] e [[Routine (Go)]]) e trouxe delegação **assíncrona** — uma task com `Assignee` diferente, ou uma rotina, cada uma rodando seu próprio turno independente via `session.Runner.Run`, não um agente chamando outro e esperando dentro do mesmo turno. Não existe hoje um turno aninhado dentro de outro — cada `Run` é uma chamada de nível superior, disparada por uma mensagem de chat, uma task ou uma rotina, nunca por uma tool call de outro agente. `SubagentStart`/`SubagentStop` continuam sem ponto de disparo porque o modelo de execução que os motivaria (um agente sincronamente chama e espera um subagente) não existe nesta arquitetura — não porque a fase que o traria ainda não chegou. Registrar isso como decisão explícita, e não como pendência de fase futura, é trabalho em aberto.
